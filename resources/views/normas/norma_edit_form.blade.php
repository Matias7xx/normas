...
<div class="card card-outline card-dark mb-4">
    ...
    <div class="card-body">
        <div class="row">
            <div class="col-lg-2 col-md-6">
                ...
                <input type="date" class="section-form-input {{ $errors->has('data') ? 'border-error' : '' }}"
                    name="data" id="data" value="{{ old('data', $norma->data ? $norma->data->format('Y-m-d') : '') }}">
            </div>
                ...
                <select class="section-form-select {{ $errors->has('publicidade') ? 'border-error' : '' }}"
                    ...
                    @foreach ($publicidades as $publicidade)
                        <option value="{{ $publicidade->id }}" 
                            {{ old('publicidade', $norma->publicidade->id) == $publicidade->id ? 'selected' : '' }}>
                ...
                <select class="section-form-select {{ $errors->has('tipo') ? 'border-error' : '' }}"
                    ...
                    @foreach ($tipos as $tipo)
                        <option value="{{ $tipo->id }}" 
                            {{ old('tipo', $norma->tipo->id) == $tipo->id ? 'selected' : '' }}>
                ...
                <select class="section-form-select {{ $errors->has('orgao') ? 'border-error' : '' }}"
                    ...
                    @foreach ($orgaos as $orgao)
                        <option value="{{ $orgao->id }}" 
                            {{ old('orgao', $norma->orgao->id) == $orgao->id ? 'selected' : '' }}>
            ...
            <div class="col-lg-4 col-md-6">
                ...
                <select class="section-form-select {{ $errors->has('vigente') ? 'border-error' : '' }}"
                    ...
                    @foreach (\App\Models\Norma::getVigenteOptions() as $value => $label)
                        <option value="{{ $value }}" 
                            {{ old('vigente', $norma->vigente) == $value ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
        ...
        <!-- Configurações de Vigência (aparece condicionalmente) -->
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="vigencia_indeterminada" 
                                        ...
                                        {{ old('vigencia_indeterminada', old('vigencia_indeterminada_hidden', $norma->vigencia_indeterminada ?? true)) == '1' ? 'checked' : '' }}
                                ...
                                @if(isset($norma->vigencia_indeterminada) && isset($norma->data_limite_vigencia))
                                    <div class="mt-2">
                                        <small class="text-info">
                                            <i class="fas fa-info-circle mr-1"></i>
                                            <strong>Configuração atual:</strong>
                                            @if($norma->vigencia_indeterminada)
                                                Vigência indeterminada
                                            @else
                                                Data limite: {{ $norma->data_limite_vigencia ? $norma->data_limite_vigencia->format('d/m/Y') : 'Não definida' }}
                                            @endif
                                        </small>
                                    </div>
                                @endif
                            ...
                            <div class="col-md-6" id="data_limite_container">
                                ...
                                <input type="date" class="section-form-input {{ $errors->has('data_limite_vigencia') ? 'border-error' : '' }}"
                                    ...
                                    value="{{ old('data_limite_vigencia', $norma->data_limite_vigencia ? $norma->data_limite_vigencia->format('Y-m-d') : '') }}"
        ...
        <div class="row">
            <div class="col-lg-8 col-md-12">
                ...
                <input type="text" class="section-form-input {{ $errors->has('descricao') ? 'border-error' : '' }}"
                    name="descricao" id="descricao" value="{{ old('descricao', $norma->descricao) }}" 
        ...
        <div class="row mt-3">
            <div class="col-12">
                ...
                <textarea class="section-form-input {{ $errors->has('resumo') ? 'border-error' : '' }}"
                    ...
                    placeholder="Informe um resumo do conteúdo da norma">{{ old('resumo', $norma->resumo) }}</textarea>
                <small class="form-text text-muted">
...
<div class="card card-outline card-dark mb-4">
    ...
    <div class="card-body">
        <div class="row">
            ...
            <div class="col-lg-7 col-md-12">
                ...
                <div class="palavras-existentes-section">
                    <div class="section-header existing-header">
                        ...
                        <span>Adicionar palavras-chave existentes</span>
                        ...
                        <select class="select2-palavras-chave section-form-select" multiple="multiple" 
                            ...
                            @foreach ($palavra_chaves as $key => $palavra_chave_obj)
                                @if (isset($array_palavra_chave) && !in_array($palavra_chave_obj->id, $array_palavra_chave))
                                    <option value="{{ $palavra_chave_obj->id }}"
                                        {{ in_array($palavra_chave_obj->id, old('add_palavra_chave', [])) ? 'selected' : '' }}>
                                        {{ $palavra_chave_obj->palavra_chave }}
                                    </option>
                                @endif
            ...
            <!-- Seção Direita - Palavras-chave Vinculadas -->
            <div class="col-lg-5 col-md-12">
                <div class="card card-secondary h-100">
                    <div class="card-header vinculadas-header">
                        <h5 class="card-title">
                            <i class="fas fa-link mr-2"></i>Palavras-chave vinculadas
                        </h5>
                    </div>
                    <div class="card-body vinculadas-body">
                        @if($norma->palavrasChave->count() > 0)
                            @foreach ($norma->palavrasChave as $key => $palavra_chave_vinculada)
                                <div class="palavra-vinculada-item">
                                    <div class="palavra-info">
                                        <i class="fas fa-tag mr-2"></i>
                                        <span class="palavra-nome">{{ $palavra_chave_vinculada->palavra_chave }}</span>
                                    </div>
                                    <button type="submit" class="btn btn-sm btn-outline-danger remover-vinculada"
                                        name="delete_palavra_chave" value="{{ $palavra_chave_vinculada->id }}"
                            ...
                            @endforeach
                        @else
                            <div class="empty-vinculadas">
                                <i class="fas fa-info-circle text-muted"></i>
                                <span class="text-muted">Nenhuma palavra-chave vinculada ainda</span>
                            </div>
                        @endif
                    </div>
                </div>
...
// Função para aplicar estado inicial baseado nos valores old() - EDIT FORM
function aplicarEstadoInicialEdit() {
    ...
    if (oldCheckboxValue !== "") {
        ...
        // Não há valor old(), usar valor da norma existente
        const normaValue = {{ $norma->vigencia_indeterminada ?? 'true' ? 'true' : 'false' }};
        checkbox.checked = normaValue;
    }
...
// Função para controlar a exibição da seção de configurações de vigência - EDIT FORM
function handleVigenciaChange() {
    ...
    if (vigenciaValue === 'VIGENTE' || vigenciaValue === 'NÃO VIGENTE') {
        ...
        updateCheckboxText();
        updateHelpText();
        // Aplicar estado inicial baseado nos valores old() ou da norma
        aplicarEstadoInicialEdit();
    } else {
        ...
        if (!hasOldValues) {
            // Resetar para valor padrão apenas se não há valores old()
            const normaValue = {{ $norma->vigencia_indeterminada ?? 'true' ? 'true' : 'false' }};
            checkbox.checked = normaValue;
            
            // Limpar e desabilitar campo de data se não há old()
            const oldDataValue = "{{ old('data_limite_vigencia', '') }}";
            if (oldDataValue === "") {
                dataInput.value = '';
            }
...
// Função para controlar o campo de data limite - EDIT FORM
...
// Inicializar quando o DOM estiver carregado - EDIT FORM
document.addEventListener('DOMContentLoaded', function() {
    handleVigenciaChange();
    toggleDataLimite();
