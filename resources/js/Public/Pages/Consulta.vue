<template>
  <PublicLayout :stats="pageStats">
    ...
    <section class="bg-white shadow-lg -mt-6 relative z-10">
      <div class="max-w-7xl mx-auto px-4 py-6 sm:py-8">
        <form @submit.prevent="buscarNormas" class="space-y-6">
          ...
          <!-- Filtros de data -->
          <div
            v-if="mostrarFiltrosData"
            class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg border"
          >
            <div>
              <label
                for="data_inicio"
    ...
    <section class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div class="flex items-center justify-between flex-wrap">
            ...
            <div class="text-sm text-blue-600">
              Página {{ normas?.current_page || 1 }} de
              {{ normas?.last_page || 1 }}
            </div>
          </div>
        </div>
      ...
      <div
        v-if="
          normas?.data && Array.isArray(normas.data) && normas.data.length > 0
        "
      ...
      <!-- Paginação -->
      <div
        v-if="normas?.data && normas.data.length > 0 && normas.last_page > 1"
        class="flex flex-col items-center space-y-4"
      >
        <!-- Navegação de páginas -->
        <nav aria-label="Navegação de páginas" class="w-full">
          <ul
            class="flex justify-center items-center space-x-1 flex-wrap gap-y-2"
          >
            <!-- Primeira página -->
            ...
            <li v-for="page in paginasVisiveisMobile" :key="page">
              ...
              <span v-else class="px-2 py-2 text-xs sm:text-sm text-gray-500"
                >...</span
              >
            </li>
        ...
        <div class="text-sm text-gray-600 text-center">
          ...
          <span class="font-semibold">{{
            (normas.current_page - 1) * normas.per_page + 1
          }}</span>
          a
          <span class="font-semibold">{{
            Math.min(normas.current_page * normas.per_page, normas.total)
          }}</span>
          de
...
const irParaPagina = pagina => {
  if (pagina === '...' || pagina === props.normas?.current_page || !pagina)
    return;
...
watch(
  () => [form.value.data_inicio, form.value.data_fim],
  ([inicio, fim]) => {
    if (inicio || fim) {
