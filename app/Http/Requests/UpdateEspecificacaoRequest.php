...
class UpdateEspecificacaoRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            'nome' => [
                ...
                'string',
                'max:255',
                Rule::unique('especificacoes')->ignore($this->route('id'))->where(function ($query) {
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($especificacao && !$especificacao->arquivo && !$this->hasFile('arquivo')) {
                $validator->errors()->add('arquivo', 'O arquivo PDF é obrigatório quando não há arquivo cadastrado.');
            }
        });
    }
}