...
class UpdateBoletimRequest extends FormRequest
{
  ...
  public function messages(): array
  {
    return [
      ...
      'data_publicacao.date' =>
        'A data de publicação deve ser uma data válida.',
  ...
  protected function failedAuthorization()
  {
    abort(
      403,
      'Acesso negado. Apenas usuários autorizados podem editar boletins.',
    );
  }
    ...
    $validator->after(function ($validator) {
      ...
      if ($this->hasFile('arquivo')) {
        ...
        if ($file && $file->getMimeType() !== 'application/pdf') {
          $validator
            ->errors()
            ->add('arquivo', 'O arquivo enviado não é um PDF válido.');
        }
        ...
        if ($file && $file->getSize() > 20971520) {
          $validator
            ...
            ->add(
              'arquivo',
              'O arquivo é muito grande. Tamanho máximo permitido: 20MB.',
            );
        }
      }
    });
