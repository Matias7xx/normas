...
class NormaSearchPublicController extends Controller
{
  public function search(Request $request)
  {
    ...
    $query = Norma::with(['orgao', 'tipo', 'palavrasChave'])->where(
      'status',
      true,
    ); // Apenas normas ativas
    ...
    $norma_pesquisa = $query
      ->orderBy('data', 'desc')
      ->orderBy('id', 'desc')
      ->paginate(15)
      ->appends($request->query());
    return view(
      'public_search.norma_search',
      compact('norma_pesquisa', 'orgaos', 'tipos'),
    );
  }
  ...
  public function searchAjax(Request $request)
  {
    try {
      ...
      // Por padrão, mostrar apenas normas VIGENTES
      if (
        !$request->hasAny(['search_term', 'tipo_id', 'orgao_id', 'vigente'])
      ) {
        $query->where('vigente', 'VIGENTE');
      } else {
        // Aplicar filtros apenas se fornecidos
  ...
  private function getAppliedFiltersInfo(Request $request)
  {
    ...
    if ($request->filled('data_fim')) {
      $filters[] =
        'Até: ' .
        \Carbon\Carbon::createFromFormat('Y-m-d', $request->data_fim)->format(
          'd/m/Y',
        );
    }
