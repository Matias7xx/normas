...
class PalavraChaveController extends Controller
{
  ...
  public function index(Request $request)
  {
    try {
      ...
      foreach ($palavras_chave as $palavra) {
        if (
          $palavra->normas_ativas_count > 0 &&
          $palavra->normasAtivas->count() === 0
        ) {
      ...
      Log::info('Lista de palavras-chave carregada', [
        ...
        'com_normas' => $palavras_chave
          ->where('normas_ativas_count', '>', 0)
          ->count(),
      ]);

      return view(
        'palavras_chaves.palavras_chaves_list',
        compact('palavras_chave'),
      );
    } catch (\Exception $e) {
  ...
  public function store(CreatePalavraChaveRequest $request)
  {
    try {
      ...
      $palavraExistente = PalavraChave::where(
        'palavra_chave',
        'LIKE',
        '%' . $request->palavra_chave . '%',
      )
        ->where('status', true)
        ->first();
      ...
      if ($palavraExistente) {
        return back()
          ...
          ->withErrors([
            'A palavra-chave "' .
            $palavraExistente->palavra_chave .
      ...
      return redirect()
        ->route('palavras_chaves.palavras_chaves_list')
        ->withSuccess(
          'Palavra-chave "' .
      ...
      return back()
        ->withInput()
        ->withErrors([
          'Erro interno no servidor. Tente novamente ou contate o administrador.',
        ]);
    }
  }
  ...
  public function edit($id)
  {
    try {
      ...
      return view(
        'palavras_chaves.palavras_chaves_edit',
        compact('palavra_chave'),
      );
    } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
      return redirect()
        ->route('palavras_chaves.palavras_chaves_list')
  ...
  public function desvincular($palavraChaveId, $normaId)
  {
    try {
      ...
      return redirect()
        ...
        ->withSuccess(
          'Palavra-chave "' .
            ($palavraChave->palavra_chave ?? 'N/A') .
            '" desvinculada da norma com sucesso!',
        );
    } catch (\Exception $e) {
      DB::rollBack();
  ...
  public function normasVinculadas($id)
  {
    try {
      ...
      $palavraChave = PalavraChave::with([
        'normasAtivas' => function ($query) {
          $query
            ->select('normas.id', 'normas.descricao', 'normas.data')
      ...
      Log::error(
        'Erro ao buscar normas vinculadas via AJAX (Eloquent): ' .
          $e->getMessage(),
        [
          'palavra_chave_id' => $id,
          'exception' => $e->getTraceAsString(),
  ...
  public function destroy($id)
  {
    try {
      ...
      Log::warning(
        'Palavra-chave excluída permanentemente com limpeza completa',
        [
          'id' => $id,
      ...
      return redirect()
        ...
        ->withSuccess(
          'Palavra-chave "' .
            $palavraChaveNome .
            '" excluída permanentemente com sucesso! (' .
            $vinculacoesRemovidas .
            ' vinculações antigas removidas)',
        );
    } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
      DB::rollBack();
      Log::warning('Tentativa de excluir palavra-chave inexistente', [
        'id' => $id,
      ]);
      return redirect()
        ->route('palavras_chaves.palavras_chaves_list')
