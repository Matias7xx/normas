...
class OrgaoController extends Controller
{
  ...
  public function show(Request $request)
  {
    try {
      ...
      return view('orgaos.orgao_list', compact('orgao'))->withErrors([
        'Erro ao carregar órgãos: ' . $e->getMessage(),
      ]);
    }
  }
    ...
    try {
      ...
      if ($normasCount > 0) {
        return redirect()
          ->route('orgaos.orgao_list')
          ->withErrors([
            "Não é possível excluir este órgão. Existem {$normasCount} norma(s) utilizando este órgão.",
          ]);
      }

      // Log da tentativa de exclusão
      Log::info(
        "Tentativa de exclusão de órgão - ID: {$id}, Nome: {$orgao->orgao}, Usuário: " .
          auth()->user()->name,
      );
