...
class NormaController extends Controller
{
  ...
  public function index(Request $request)
  {
    try {
      ...
      $userPermissions = [
        ...
        'canDelete' =>
          $user->role_id == 1 || in_array($user->role_id, [1, 2, 3]),
        'canCreate' =>
          $user->role_id == 1 || in_array($user->role_id, [1, 2, 3]),
      ...
      return view(
        'normas.norma_list',
        compact('tipos', 'orgaos', 'userPermissions'),
      );
    } catch (\Exception $e) {
      Log::error('Erro ao listar normas: ' . $e->getMessage());
      return back()->withErrors([
        'Erro ao carregar normas. Por favor, tente novamente.',
      ]);
    }
  }
    ...
    try {
      ...
      $formattedNormas = $normas->map(function ($norma) use ($showAudit) {
        $normaData = [
          ...
          'palavras_chave' => $norma->palavrasChave
            ->take(3)
            ->map(function ($pc) {
              return ['id' => $pc->id, 'palavra_chave' => $pc->palavra_chave];
            }),
          'palavras_chave_restantes' =>
            $norma->palavrasChave->count() > 3
              ? $norma->palavrasChave->count() - 3
              : 0,
        ];
        ...
        if ($showAudit && $norma->usuario) {
          $normaData['auditoria'] = [
            ...
            'data_cadastro' => $norma->created_at
  ...
  private function getAppliedFiltersInfo(Request $request)
  {
    ...
    if ($request->filled('data_inicio')) {
      $filters[] =
        'A partir de: ' .
        Carbon::createFromFormat('Y-m-d', $request->data_inicio)->format(
          'd/m/Y',
        );
    }
    ...
    if ($request->filled('data_fim')) {
      $filters[] =
        'Até: ' .
        Carbon::createFromFormat('Y-m-d', $request->data_fim)->format('d/m/Y');
    }
  ...
  public function create()
  {
    try {
      $tipos = Tipo::where('status', true)->orderBy('tipo')->get();
      $publicidades = Publicidade::where('status', true)
        ->orderBy('publicidade')
        ->get();
      $orgaos = Orgao::where('status', true)->orderBy('orgao')->get();
      ...
      return view(
        'normas.norma_create',
        compact('tipos', 'orgaos', 'publicidades', 'palavras_chave'),
      );
    } catch (\Exception $e) {
  ...
  public function edit($id)
  {
    try {
      ...
      $publicidades = Publicidade::where('status', true)
        ...
        ->get();
      $orgaos = Orgao::where('status', true)->orderBy('orgao')->get();
      $palavra_chaves = PalavraChave::where('status', true)
        ->orderBy('palavra_chave')
        ->get();

      return view(
        'normas.norma_edit',
        compact('norma', 'tipos', 'orgaos', 'publicidades', 'palavra_chaves'),
      );
    } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
      return redirect()
        ->route('normas.norma_list')
        ->withErrors(['Norma não encontrada.']);
    } catch (\Exception $e) {
      Log::error('Erro ao editar norma: ' . $e->getMessage());
      return back()->withErrors([
        'Erro ao carregar formulário de edição. Por favor, tente novamente.',
      ]);
    }
  }
  ...
  public function store(CreateNormaRequest $request)
  {
    ...
    try {
      ...
      $nameFile = $this->generateNormaFileName(
        $request->descricao,
        $file->extension(),
      );
      ...
      if (
        $request->has('palavras_chave') &&
        is_array($request->palavras_chave)
      ) {
        $palavrasChaveIds = array_merge(
          $palavrasChaveIds,
      ...
      if (
        $request->has('novas_palavras_chave') &&
        !empty($request->novas_palavras_chave)
      ) {
        try {
          $novasPalavrasChave = json_decode(
            $request->novas_palavras_chave,
            true,
          );
          ...
          Log::error(
            'Erro ao processar novas palavras-chave: ' . $e->getMessage(),
          );
        }
      }
  ...
  public function update(UpdateNormaRequest $request, $id)
  {
    ...
    try {
      // Obter a norma
      $norma = Norma::where('id', $id)->ativas()->firstOrFail();
      ...
      // Processar palavras-chave existentes selecionadas
      if (
        $request->has('add_palavra_chave') &&
        is_array($request->add_palavra_chave) &&
        count($request->add_palavra_chave) > 0
      ) {
      ...
      if (
        $request->has('vigente') &&
        in_array($request->vigente, array_keys(Norma::getVigenteOptions()))
      ) {
      ...
      if ($request->hasFile('anexo') && $request->file('anexo')->isValid()) {
        ...
        // Usar descrição da norma para gerar nome do arquivo (MinIO)
        $descricaoParaNome = $request->has('descricao')
          ? $request->descricao
          : $norma->descricao;
        $nameFile = $this->generateNormaFileName(
          $descricaoParaNome,
          $request->anexo->extension(),
        );
        ...
        StorageHelper::normas()->putFileAs(
          '/',
          $request->file('anexo'),
          $nameFile,
        );
      ...
      if (
        ...
        $request->descricao !== $norma->getOriginal('descricao') &&
        $norma->anexo &&
        !$request->hasFile('anexo')
      ) {
        try {
          ...
          $novoNome = $this->generateNormaFileName(
            $request->descricao,
            $extensaoAtual,
          );
  ...
  public function destroy($id)
  {
    try {
      ...
      if (!in_array(auth()->user()->role_id, [1, 2, 3])) {
        return back()->withErrors([
          'Você não tem permissão para excluir normas.',
        ]);
      }
      ...
      if (
        request()->has('preserve_filters') &&
        request()->preserve_filters == '1'
      ) {
      ...
      return back()->withErrors([
        'Erro ao remover norma. Por favor, tente novamente.',
      ]);
    }
  }
  ...
  public function normasDuplicadas(Request $request)
  {
    ...
    $duplicadas_paginadas = array_slice(
      $duplicadas,
      $offset,
      $grupos_por_pagina,
    );
  ...
  public function removerVerificacao(Request $request, $id): JsonResponse
  {
    try {
      ...
      return response()->json([
        'success' => true,
        'message' =>
          'Verificação removida. O grupo voltará a aparecer na lista de duplicadas.',
      ]);
    } catch (\Exception $e) {
      return response()->json(
        [
  ...
  private function calcularSimilaridadeExata($texto1, $texto2)
  {
    ...
    $distancia = levenshtein(substr($texto1, 0, 255), substr($texto2, 0, 255));
