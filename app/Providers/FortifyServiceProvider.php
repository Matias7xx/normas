...
class FortifyServiceProvider extends ServiceProvider
{
  ...
  public function boot()
  {
    ...
    Fortify::authenticateUsing(function (Request $request) {
      ...
      if ($this->isUsuarioAdministrativo($user)) {
        Log::info(
          "Usuário administrativo {$user->matricula} - autenticação apenas local",
        );
    ...
    try {
      ...
      if ($response->successful()) {
        ...
        $user->update([
          'name' => $usuario['nome'],
          'email' =>
            $usuario['email'] ?: $usuario['matricula'] . '@pc.pb.gov.br',
          'status' => $usuario['status'],
          'unidade_lotacao_id' =>
            $usuario['lotacao_principal']['codigo_unidade_lotacao'] ??
            $user->unidade_lotacao_id,
      ...
      Log::warning(
        "Falha na autenticação via API para {$user->matricula}: " .
          $e->getMessage(),
      );
    }
    ...
    try {
      ...
      if ($response->successful()) {
        ...
        $user = User::create([
          ...
          'unidade_lotacao' => $this->getLotacaoServidor($usuario),
          'unidade_lotacao_id' =>
            $usuario['lotacao_principal']['codigo_unidade_lotacao'] ?? 340,
    ...
    try {
      ...
      if ($response->successful()) {
        ...
        $user->update([
          ...
          'classe_funcional' =>
            $usuario['classe_funcional'] ?? $user->classe_funcional,
          'nivel_funcional' => $usuario['nivel'] ?? $user->nivel_funcional,
        ]);

        Log::info(
          "Dados funcionais atualizados via API para usuário {$user->matricula}",
        );
      }
    } catch (\Exception $e) {
      Log::debug(
        "Não foi possível atualizar dados via API para usuário {$user->matricula}: " .
          $e->getMessage(),
      );
    }
  }
  ...
  {
    if (
      isset($servidor['lotacao_principal']) &&
      $servidor['lotacao_principal'] != null
    ) {
      return $servidor['lotacao_principal']['unidade_lotacao'];
    } elseif (
      isset($servidor['orgao_cedido']) &&
      isset($servidor['orgao_cedido']['nome'])
    ) {
