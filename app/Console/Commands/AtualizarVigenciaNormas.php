...
class AtualizarVigenciaNormas extends Command
{
  ...
  public function handle()
  {
    ...
    $this->info(
      "Encontradas {$normasParaVerificar->count()} normas para verificação:",
    );
    ...
    foreach ($normasParaVerificar as $norma) {
      try {
        ...
        if ($novoStatus === $statusAtual) {
          $this->line(
            "ID {$norma->id}: {$norma->descricao} - Já está no status correto ({$statusAtual})",
          );
          continue;
        }
        ...
        $this->line(
          '   Data limite: ' . $norma->data_limite_vigencia->format('d/m/Y'),
        );
    ...
    if ($dryRun && $contadorAtualizacoes > 0) {
      $this->newLine();
      $this->warn(
        'Para executar as alterações, execute o comando sem --dry-run:',
      );
